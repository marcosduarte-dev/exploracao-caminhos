...
class ReportDialog(QDialog):
    def __init__(self, db_connection, local_stats, parent=None):
        ...
        self.table.setHorizontalHeaderLabels(["Algoritmo", "Tamanho", "Tempo (ms)", "Células Visitadas", "Caminho", "Memória (MB)"])
    ...
    def populate_table(self):
        ...
        for row, stat in enumerate(self.stats):
            ...
            self.table.setItem(row, 0, QTableWidgetItem(algorithm_name))
            self.table.setItem(row, 1, QTableWidgetItem(size))
            self.table.setItem(row, 2, QTableWidgetItem(time_val))
            self.table.setItem(row, 3, QTableWidgetItem(visited_val))
            self.table.setItem(row, 4, QTableWidgetItem(path_val))
            self.table.setItem(row, 5, QTableWidgetItem(mem_val))
